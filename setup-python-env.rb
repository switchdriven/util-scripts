...
class SetupPythonEnv
  ...
  def parse_arguments
    parser = OptionParser.new do |opts|
      opts.banner = "Usage: setup-python-env.rb [OPTIONS] [PROJECT_DIR]"
      opts.separator ""
      opts.separator "Sets up a Python development environment with uv and direnv."
      opts.separator ""
      ...
      opts.on("-v", "--python-version VERSION", "Python version to use (default: #{DEFAULT_PYTHON_VERSION})") do |v|
        @python_version = v
      end
      ...
      opts.separator "  #{File.basename($0)} --python-version 3.12     # Use Python 3.12"
      opts.separator "  #{File.basename($0)} --mcp work                # Setup with work GitHub MCP config"
      opts.separator ""
      opts.separator "REQUIREMENTS:"
      opts.separator "  - uv: Python package installer (https://github.com/astral-sh/uv)"
  ...
  def create_venv
    ...
    system("uv venv #{@venv_dir} --python #{@python_version}")
  ...
  def print_summary
    ...
    puts "  2. Install dependencies: uv pip install <package>"
    puts "  3. Start coding!"
    puts ""
...
# Run the setup
SetupPythonEnv.new.run if __FILE__ == $0
