...
**注**:
- `net-port.rb` でも SSID 取得機能があります。
- ショートカット「GetSSID」経由で取得するため、実行中にクリップボードの内容が上書きされます。
...
社内ネットワークの総合診断ツール。Wi-Fi SSID・インターフェース状態・L3疎通（ARP）・DNS・Proxy・Web到達性をまとめてチェックする。
...
PDF内の埋め込み画像を再圧縮・ダウンサンプリングしてファイルサイズを縮小するスクリプト。テキストやベクター部分はラスタライズせず維持します。
...
`mailto:` URL のデフォルトメールアプリを切り替えるスクリプト。`mail` / `spark` のエイリアスに加え、任意のバンドルIDも指定可能。

```bash
./set-mailto-app.sh --help
```

### chrome-profiles.rb

Chrome / Edge などの Chromium 系ブラウザのプロファイル名とフォルダ名を一覧表示する。`--launch` でプロファイルを指定して起動も可能。

```bash
./chrome-profiles.rb --help
```

### update-all.sh

Homebrew・npm（グローバル）・uv（グローバル仮想環境）・uv tool（グローバルツール）・App Store・Claude Code・Apple Container の更新有無を確認し、対話的にアップデートする一括スクリプト。`~`（ホームディレクトリ）で実行することを想定しており、`~/Scripts/.venv` のグローバル仮想環境を対象にする。なお uv（グローバル仮想環境）はこのリポジトリのスクリプトが直接依存するため、自動アップグレードは行わず確認表示のみとしている。
