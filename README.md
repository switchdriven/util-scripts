...
Homebrew・npm（グローバル）・uv（グローバル仮想環境）・uv tool（グローバルツール）・App Store・Claude Code・Apple Container の更新有無を確認し、対話的にアップデートする一括スクリプト。`~`（ホームディレクトリ）で実行することを想定しており、`~/Scripts/.venv` のグローバル仮想環境を対象にする。
